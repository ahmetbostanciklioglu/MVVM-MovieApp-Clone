...
class NetworkManager {
    ...
    typealias competionHandler2 = (UIImage?, String?)-> Void
    func getImage(fromUrl url: String, completion: @escaping  competionHandler2) {
        ...
        request.responseData { (response) in
            ...
            case .success:
                guard let imageData = response.value,
                      let image = UIImage(data: imageData, scale: 1.0) else {
                    return
                }
